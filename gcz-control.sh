...
main_menu() {
  ...
  echo "+----------------------------------------------+"
  echo "VPS      : ONLINE"
  echo -n "PM2      : "; pm2 list | grep -E "gcz-(api|bot|redirect|watchdog)" | awk '{print $4"="$10}' | paste -sd'  ' -
  ...
  echo "[4] PM2 list"
  echo "[5] Logs (all/api/site/bot)"
  echo "[6] Health detail (curl)"
  echo "[7] Settings"
  echo "[8] Next page"
  echo "[9] Auto-refresh view"
  echo "[R] Check Redirect Menu"
  echo "[0] Exit"
  echo -n "Select: "
  read choice
