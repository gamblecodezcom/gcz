...
$logEntry = ['type' => 'sale', 'timestamp' => date('c'), 'raw_data' => $input, 'headers' => getallheaders()];
$logs[] = $logEntry;
...
if ($data = json_decode($input, true)) {
    ...
    $conversion = ['id' => $data['id'] ?? uniqid(), 'type' => 'sale', 'eventDate' => $data['eventDate'] ?? date('c'), 'affiliateClickId' => $data['affiliateClickId'] ?? '', 'offerId' => $data['offerId'] ?? '', 'dealId' => $data['dealId'] ?? '', 'countryCode' => $data['countryCode'] ?? '', 'state' => $data['state'] ?? '', 'amount' => $data['amount'] ?? 0, 'commission' => $data['commission'] ?? 0];
    $conversions[] = $conversion;
    file_put_contents($conversionsFile, json_encode($conversions, JSON_PRETTY_PRINT), LOCK_EX);
}
http_response_code(200);
echo json_encode(['status' => 'ok', 'message' => 'Sale postback received']);
?>