#!/usr/bin/env python3
"""
GCZ AI Watchdog — Sandbox
Same logic as production but isolated to sandbox namespace and PM2 targets.
"""
...
logger = get_logger("gcz.sandbox_ai_watchdog")
...
def _record_memory(category: str, message: str, meta: Dict[str, Any]) -> None:
    ...
    gcz_ai.execute(
        ...
        """,
        [category, message, "sandbox_ai_watchdog", meta],
    )
...
def _pm2_restart_targets() -> None:
    for name in PM2_TARGETS:
        if not name.startswith("gcz-sandbox-"):
            logger.warning("Skipping non-sandbox PM2 target", extra={"service": name})
            continue
...
def run_cycle() -> None:
    ...
    if not healthy:
        ...
        _record_memory("sandbox.watchdog.failure", "health_degraded", meta)
        _record_anomaly("sandbox_watchdog_failure", "Sandbox AI watchdog health degraded", meta)
        ...
        if fails:
            _record_memory("sandbox.watchdog.recovery", "health_recovered", {"env": ENV})
        fails = 0
...
def main() -> None:
    if ENV != "sandbox":
        logger.error("Sandbox AI watchdog started outside sandbox", extra={"env": ENV})
        return

    logger.info("Sandbox AI watchdog started", extra={"env": ENV})
